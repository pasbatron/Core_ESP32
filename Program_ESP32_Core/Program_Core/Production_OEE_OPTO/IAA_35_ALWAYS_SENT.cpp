...
unsigned long lastSignalSendTime = 0;
const unsigned long signalSendInterval = 1500; // 1.5 seconds
...
void sendSignalStates() {
  String signalStates = "States: A2=" + String(buttonStateA2) + ", A3=" + String(buttonStateA3) +
                        ", A4=" + String(buttonStateA4) + ", A5=" + String(buttonStateA5) +
                        ", A13=" + String(buttonStateA13) + ", A14=" + String(buttonStateA14) +
                        ", A15=" + String(buttonStateA15);
  mySerialLora.println(signalStates);
}
...
void loop() {
  ...
  // Send button states every 1.5 seconds
  if (millis() - lastSignalSendTime >= signalSendInterval) {
    sendSignalStates();
    lastSignalSendTime = millis();
  }

  delay(200); // Short delay to avoid unnecessary CPU usage
}
